    ...
    <main style={{ minHeight: "100vh", display: "grid", placeItems: "center", padding: "2rem" }}>
      <div style={{ width: "100%", maxWidth: "460px", border: "1px solid rgba(0,0,0,0.1)", borderRadius: "18px", padding: "2rem", display: "grid", gap: "1rem", background: "linear-gradient(180deg, rgba(255,255,255,0.97) 0%, rgba(248,248,248,0.97) 100%)", boxShadow: "0 14px 34px rgba(0,0,0,0.1)" }}>
        ...
        <form onSubmit={handleSubmit} style={{ display: "grid", gap: "0.85rem" }}>
          <input type="password" autoComplete="new-password" value={password} onChange={(event) => setPassword(event.target.value)} placeholder="New password" required disabled={!token} style={{ width: "100%", border: "1px solid rgba(0,0,0,0.22)", borderRadius: "12px", padding: "0.8rem 0.9rem", font: "inherit", background: "#f5f6f8", boxShadow: "inset 0 1px 2px rgba(0,0,0,0.04)" }} />
          <input type="password" autoComplete="new-password" value={confirmPassword} onChange={(event) => setConfirmPassword(event.target.value)} placeholder="Confirm new password" required disabled={!token} style={{ width: "100%", border: "1px solid rgba(0,0,0,0.22)", borderRadius: "12px", padding: "0.8rem 0.9rem", font: "inherit", background: "#f5f6f8", boxShadow: "inset 0 1px 2px rgba(0,0,0,0.04)" }} />
